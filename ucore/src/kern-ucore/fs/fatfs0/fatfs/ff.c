...
)
{
	...
	if (vol >= _VOLUMES) {				/* Is the drive number valid? */
		//FAT_PRINTF("in check if valid: %d >= %d\n", vol, _VOLUMES);
	...
	*rfs = fs = FatFs[vol];				/* Return pointer to the corresponding file system object */
	...
	//FAT_PRINTF("The logical drive must be mounted.\n");
	...
	//FAT_PRINTF("Search FAT partition on the drive. Supports only generic partitionings, FDISK and SFD.\n");
	...
	//FAT_PRINTF("Following code initializes the file system object\n");
	
	if (LD_WORD(fs->win+BPB_BytsPerSec) != SS(fs)) {	/* (BPB_BytsPerSec must be equal to the physical sector size) */
		FAT_PRINTF("(BPB_BytsPerSec must be equal to the physical sector size)\n");
	...
	//FAT_PRINTF("Determine the FAT sub type\n");
...
)
{
	...
	if (res == FR_OK) {
		...
		FAT_PRINTF("chk_mounted failed, %d\n", res);
	}
